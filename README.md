...
As well as the Google Shell Style Guide:

https://google.github.io/styleguide/shell.xml

If anything is not mentioned explicitly in this guide, it defaults to matching
whatever is outlined in one of the sources above.
...
Bash is not C, semicolons are NOT necesarry to end a statement.
...
The exception to this rule is outlined in the `Block Statements` section below (and even then still not totall necesarry).
